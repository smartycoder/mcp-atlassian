...
async def get_jira_fetcher(ctx: Context) -> JiraFetcher:
    ...
    # Check contextvar for user authentication (set by middleware)
    user_auth_ctx = user_auth_context.get()
    if user_auth_ctx and user_auth_ctx.token and user_auth_ctx.auth_type:
        user_auth_type = user_auth_ctx.auth_type
        user_token = user_auth_ctx.token
        user_email = user_auth_ctx.email
        user_cloud_id = user_auth_ctx.cloud_id

        logger.debug(
            f"get_jira_fetcher: Found user auth in contextvar. auth_type='{user_auth_type}', "
            f"token_present=True, cloud_id='{user_cloud_id}'"
        )
        ...
        logger.debug(
            f"get_jira_fetcher: No user auth in contextvar. Will use global fallback."
        )
...
async def get_confluence_fetcher(ctx: Context) -> ConfluenceFetcher:
    ...
    if user_auth_ctx and user_auth_ctx.token and user_auth_ctx.auth_type:
        ...
        logger.debug(
            f"get_confluence_fetcher: Found user auth in contextvar. auth_type='{user_auth_type}', "
            f"token_present=True, cloud_id='{user_cloud_id}'"
        )
        ...
        logger.debug(
            f"get_confluence_fetcher: No user auth in contextvar. Will use global fallback."
        )
    lifespan_ctx_dict_global = ctx.request_context.lifespan_context  # type: ignore
    app_lifespan_ctx_global: MainAppContext | None = (
        lifespan_ctx_dict_global.get("app_lifespan_context")
